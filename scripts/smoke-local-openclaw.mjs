  ...
  return new Promise((resolve, reject) => {
    ...
    srv.once('error', (err) => {
      ...
      if (code === 'EPERM' || code === 'EACCES') {
        reject(new Error(`Port probe blocked by OS policy (${code}).`));
        return;
      }
      resolve(false);
    });
...
async function main() {
  ...
  try {
    ...
    await waitForHttp(`${baseUrl}/workspaces`, 25000);
    log('services are up');

    const workspaces = await apiRequest(baseUrl, 'GET', '/workspaces');
    if (!Array.isArray(workspaces) || workspaces.length === 0) throw new Error('No workspaces in seed data');
    const workspaceId = String(workspaces[0].id);
    const boards = await apiRequest(baseUrl, 'GET', `/workspaces/${encodeURIComponent(workspaceId)}/boards`);
    if (!Array.isArray(boards) || boards.length === 0) throw new Error('No boards in workspace');
    ...
    const members = await apiRequest(baseUrl, 'GET', `/workspaces/${encodeURIComponent(workspaceId)}/members`);
    if (!Array.isArray(members) || members.length === 0) {
      throw new Error('Workspace members are not accessible');
    }
    log(`workspace members: ${members.length}`);

    const memory = await apiRequest(
      baseUrl,
      'PUT',
      '/memory/docs',
      { scope: 'board', id: boardId, content: '# Smoke Context\\n\\nHello from smoke.' }
    );
    if (!memory?.ok) throw new Error('Memory board update failed');
    log('board memory updated');
